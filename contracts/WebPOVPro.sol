 ...
 * @dev A smart contract that allows users to join a DAP by sending 0.001 ether and leave the DAP to withdraw their funds.
...
contract WebPOVProV0 is Ownable {
    ...
    uint256 public constant JOINING_FEE = 0.001 ether;
    ...
    /**
     * @dev Allows users to join the DAP by sending exactly 0.001 ether.
     * @notice Users must send exactly 0.001 ether to become a member of the DAP.
     */
    function joinWebPovProtocolV0() external payable {
        require(msg.value == JOINING_FEE, "Please send exactly 0.001 ether.");
